  ...
  <nav class="navbar navbar-default">
    <div class="container-fluid">
      <div class="navbar-header">
        ...
        <button class="navbar-toggle visible-xs" type="button" data-toggle="push" data-target="#navbar-main"
        ...
        <a href="{{ url('/') }}" class="navbar-brand" style="padding:5px 15px; ">
          <img src="{{url('/logo.png')}}" style="max-height:90%;" />
        </a>
      </div>
        ...
        <ul class="nav navbar-nav">
          <li class="dropdown">
            <a class="dropdown-toggle" data-toggle="dropdown" href="#"
              id="themes">{{trans('strings.frontend.categories')}} <span class="caret"></span></a>
            <ul class="dropdown-menu" aria-labelledby="themes">
              <li><a href="{{route('frontend.course.get')}}?category=">{{trans('strings.frontend.all-categories')}}</a>
              </li>
        ...
        <div class="mk-fullscreen-search-overlay" id="mk-search-overlay">
          ...
          <div id="mk-fullscreen-search-wrapper">
            <form action="{{ route('frontend.course.get') }}" method="get" id="mk-fullscreen-searchform">
              <input type="text" autocomplete="off" name="search" class="main-search"
                placeholder="{{trans('strings.frontend.search-author-or-title')}}..." id="mk-fullscreen-search-input">
        ...
        <ul class="nav navbar-nav navbar-right">
          ...
          <li class="dropdown">
            ...
            <ul class="dropdown-menu notify-drop">
              <div class="notify-drop-title">
                <div class="row">
                  <div class="col-md-6 col-sm-6 col-xs-6">{{trans('strings.frontend.unread')}}
                    (<b>{{count(auth()->user()->unreadNotifications)}}</b>)</div>
                </div>
              </div>
              <!-- end notify title -->
              <!-- notify content -->
              ...
              <div class="notify-drop-footer text-center">
                ...
                <a href="#" id="mark-all-as-read" class="mark-all-as-read"><i class="fa fa-eye"></i>
                  {{trans('strings.frontend.mark-all-as-read')}}</a>
              </div>
            </ul>
          </li>
          ...
          <li class="dropdown">
            ...
            <ul class="dropdown-menu" aria-labelledby="download">
              ...
              @endif
              <li><a
                  href="{{ route('frontend.user.public.profile', auth()->user()) }}">{{trans('strings.frontend.profile')}}</a>
              </li>
              <li><a href="{{route('frontend.user.account')}}">{{trans('navs.frontend.user.account')}}</a></li>
              <li><a href="{{ route('frontend.user.messages', Auth::user()) }}">{{trans('strings.frontend.messages')}}</a>
              </li>
              ...
              <li class="divider"></li>
              <li><a
                  href="{{route('frontend.author.course.index')}}">{{ trans('navs.frontend.instructor_dashboard') }}</a>
              </li>
              <!--
                        <li><a href="{{route('frontend.author.revenue.index')}}">{{trans('strings.frontend.revenue-report')}}</a></li>
                        -->
              <li><a href="{{route('frontend.author.course.create')}}">{{trans('strings.frontend.create-new-course')}}</a>
              </li>
              @endif
