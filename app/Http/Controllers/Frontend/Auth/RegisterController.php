...
class RegisterController extends Controller
{
    ...
    public function register(RegisterRequest $request)
    {
        ...
        if (config('access.users.confirm_email')) {
            $user = $this->user->create($request->only('first_name', 'last_name', 'username', 'email', 'password'));
            event(new UserRegistered($user));

            return redirect($this->redirectPath())->withFlashSuccess(trans('exceptions.frontend.auth.confirmation.created_confirm'));
        } else {
            access()->login($this->user->create($request->only('first_name', 'last_name', 'username', 'email', 'password')));
