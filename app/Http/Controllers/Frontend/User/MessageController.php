...
class MessageController extends Controller
{
    ...
    {
        //$test = $this->uploadImage($request->image);
        $test = $request->body;
    ...
    protected function uploadImage($string, $width = 800, $height = 600)
    {
        ...
        $processedImage = $this->imageManager->make($img)
            ->fit($width, $height, function ($c) {
                $c->aspectRatio();
            })
            ->encode('jpg')
            ->save(public_path('uploads/images/chat/' . $filename = uniqid() . '.jpg'));
